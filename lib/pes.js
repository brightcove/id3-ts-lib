 ...
 */
exports.generatePESHeader = (buffer, destination, pts, dataLength, headerPaddingLength) => {
  ...
  if (Number.isFinite(pts)) {
    if (buffer.length - destination < 14) {
      throw new RangeError('The amount of destination buffer remaining is less than the 14 bytes required for a PES header with a PTS.');
    }
    ...
    throw new RangeError('The amount of destination buffer remaining is less than the 9 bytes required for a PES header.');
  }
