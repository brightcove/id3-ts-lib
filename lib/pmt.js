...
exports.generatePMTPacket = (buffer, destination, options) => {
  ...
  if (!Number.isFinite(pmtPid) || pmtPid < 4 || pmtPid >= 0x1fff) {
    throw new Error('The "pmtPid" option is required and must be a number in the range of 4 to 8190.');
  }
  if (!Number.isFinite(id3Pid) || id3Pid === pmtPid) {
    throw new Error('The "id3Pid" option is required and must be a number in the range of 4 to 8190 and can not equal the "pmtPid".');
  }
  if (buffer.length - destination < 188) {
    throw new Error('The amount of destination buffer remaining is less than the 188 bytes required for a PMT packet.');
  }
