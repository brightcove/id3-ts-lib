...
exports.generatePMTPacket = (buffer, destination, options) => {
  ...
  if (!Number.isFinite(id3Pid) || id3Pid < 4 || id3Pid >= 0x1fff || id3Pid === pmtPid) {
    throw new TypeError('The "id3Pid" option is required and must be a number in the range of 4 to 8190 and can not equal the "pmtPid".');
  }
  if (buffer.length - destination < 188) {
    throw new RangeError('The amount of destination buffer remaining is less than the 188 bytes required for a PMT packet.');
  }
  ...
  // Write the program_info descriptors since the ID3 PID is required
